...
Quando `now >= exit_deadline_at`, a reconciliação registra a saída automaticamente, encerra a sessão e a alocação em `exit_deadline_at`, define `TIME_LIMIT_REACHED` e registra `SYSTEM_ADMIN` como perfil de saída. A ADR 0026 acrescenta a reconciliação anterior às consultas operacionais; o comando `reconcile_operational_deadlines` permanece ativo, em execução única ou contínua, para cobrir períodos sem acesso.
...
- a API mantém os deadlines para operação e auditoria; a área do Usuário da Sala usa a janela internamente para habilitar ações, mas não exibe seus limites;
